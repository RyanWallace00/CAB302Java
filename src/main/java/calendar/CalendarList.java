...
public class CalendarList {
    ...
    public void onShowAllItems() {
        for (int i = 0; i < user.getCalendarItems().size(); i++) {
            CalendarItem item= user.getCalendarItems().get(i);
            if (item.isDone()) {
    ...
    public void onMarkAsDone() {
        ...
        boolean exists = user.ifCalendarItemExists(itemNumber);
        if (exists){
            user.getCalendarItems().get(itemNumber).setDone(true);
    ...
    public void onMarkAsUndone() {
        ...
        boolean exists = user.ifCalendarItemExists(itemNumber);
        if (exists){
            user.getCalendarItems().get(itemNumber).setDone(false);
    ...
    public void onDeleteItem() {
        ...
        boolean exists = user.ifCalendarItemExists(itemNumber);
        if (exists){
            user.getCalendarItems().remove(itemNumber);
